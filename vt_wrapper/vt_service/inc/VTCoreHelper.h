...
class VTCoreHelper : virtual public RefBase {
  public:
    ...
    void logString(const char* str, VT_SRV_LOG_LV level);
    void logAction(const char* action, const char* des, int data, const char* data2,
                   VT_SRV_LOG_LV level);
    void logFlow(const char* flow, const char* des, int data, const char* data2,
                 VT_SRV_LOG_LV level);
    void logMap(const char* action, const char* des, int data, VT_SRV_LOG_LV level);
    void logMsg(int notify, const char* des, int id, int data1, int data2, int data3,
  ...
  private:
    ...
    const char mCallModeString[VT_SRV_CALL_END][50] = {"Call mode : None", "Call mode : 3G",
                                                       "Call mode : 4G"};
    ...
    const char mRetString[VT_SRV_RET_END][50] = {"Operation success",
    ...
    const char mDataModeString[VT_SRV_DATA_PATH_END][50] = {"Path : none", "Path : sink",
                                                            "Path : source", "Path : source sink"};
    const char mCameraString[3][50] = {"Turn off camera", "camera id 0", "camera id 1"};
    const char mHideMeString[VT_SRV_HIDE_ME_END][50] = {"Hide me : Disable", "Hide me : Enable",
    ...
    const char mSnapshotString[VT_SRV_SNAPSHOT_END][50] = {"Snapshot local", "Snapshot peer",
                                                           "Snapshot local & peer"};
    const char mRecordString[VT_SRV_RECORD_END][50] = {"Record video", "Record video / audio"};
    const char mVideoStateString[8][50] = {"Video State : audio",
    ...
    const char mCallTypeString[2][50] = {"Call Mode : voice", "Call Mode : video"};
    const char mOnOffString[2][50] = {"OFF", "ON"};
    const char mLocalCameraStateString[2][50] = {"Local Camera State : ON",
                                                 "Local Camera State : OFF"};
    const char mPeerCameraStateString[2][50] = {"Peer Camera State : ON",
                                                "Peer Camera State : OFF"};
    const char mHOStateString[2][50] = {"False", "True"};
    const char mSessionModifyResultString[11][50] = {"Success",
