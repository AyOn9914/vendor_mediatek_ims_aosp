...
void* VectorImpl::editArrayImpl()
{
    if (mStorage) {
        ...
        if (editable == 0) {
            ...
            LOG_ALWAYS_FATAL_IF(editable == NULL, "Assert");
...
ssize_t VectorImpl::setCapacity(size_t new_capacity)
{
    ...
    LOG_ALWAYS_FATAL_IF(!safe_mul(&new_allocation_size, new_capacity, mItemSize), "Assert");
...
//        this, (int)where, (int)amount, (int)mCount, (int)capacity());
    ...
    size_t new_size;
    LOG_ALWAYS_FATAL_IF(!safe_sub(&new_size, mCount, amount), "Assert");
