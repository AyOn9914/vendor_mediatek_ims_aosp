...
VTCore::~VTCore() {
    ...
    mHelper->logFlow("~VTCore ", " error ~VTCore() here, abort to get backtrace", VT_IVD, "",
                     VT_SRV_LOG_E);
    abort();
}
...
status_t VTCore::initialization(int mode, int id, int sim_id) {
    if (!mHelper->getIsSetSensorInfo()) {
        ...
        notifyCallback(id, VT_SRV_NOTIFY_GET_SENSOR_INFO, 0, 0, 0, String8(""), String8(""), NULL);
    }
...
status_t VTCore::open(VT_SRV_CALL_MODE mode, const int id, const int sim_id) {
    ...
    mLastRet = mHelper->getParam(id, VT_SRV_PARAM_IS_FORCE_STOP,
                                 reinterpret_cast<void**>(&isForceStop));
    if (mLastRet) goto error;
    ...
    mLastRet =
            mHelper->getParam(id, VT_SRV_PARAM_IS_MA_CRASH, reinterpret_cast<void**>(&isMaCrash));
    if (mLastRet) goto error;
...
status_t VTCore::init_internal(const int id) {
    ...
    if (VT_SRV_CALL_4G == mode) {
        ...
        // Config MA
        mLastRet =
                mHelper->getParam(id, VT_SRV_PARAM_MA_CONFIG, reinterpret_cast<void**>(&ma_config));
        if (mLastRet) goto error;
...
status_t VTCore::updateCallMode(const int id) {
    ...
    mHelper->logFlow("updateCallMode", "ua->setting.mode", id,
                     mHelper->getCallModeString(ua->setting.mode), VT_SRV_LOG_I);
    mHelper->logFlow("updateCallMode", "info->mIsHold", id, mHelper->getOnOffString(info->mIsHold),
                     VT_SRV_LOG_I);
    mHelper->logFlow("updateCallMode", "info->mIsHeld", id, mHelper->getOnOffString(info->mIsHeld),
                     VT_SRV_LOG_I);
    mHelper->logFlow("updateCallMode", "info->mIsResume", id,
                     mHelper->getOnOffString(info->mIsResume), VT_SRV_LOG_I);
    mHelper->logFlow("updateCallMode", "info->mIsResumed", id,
                     mHelper->getOnOffString(info->mIsResumed), VT_SRV_LOG_I);
...
status_t VTCore::updateTxRxMode(const int id, int new_mode, vt_srv_call_update_info_struct* info) {
    ...
    mHelper->logFlow("updateTxRxMode", "new_mode", id, mHelper->getVideoStateString(new_mode),
                     VT_SRV_LOG_I);
    ...
    mHelper->logFlow("updateTxRxMode", "curr_state", id, mHelper->getStateString(curr_state),
                     VT_SRV_LOG_I);
    ...
    do {
        ...
        mLastRet = mHelper->getParam(id, VT_SRV_PARAM_IS_MA_CRASH,
                                     reinterpret_cast<void**>(&isMaCrash));
        ...
        mLastRet = mHelper->getParam(id, VT_SRV_PARAM_IS_FORCE_CANCEL,
                                     reinterpret_cast<void**>(&isForceCancel));
        ...
        mLastRet = mHelper->getParam(id, VT_SRV_PARAM_SURFACE_STATE,
                                     reinterpret_cast<void**>(&setSfState));
        if (mLastRet) goto error;

        mHelper->logFlow("updateTxRxMode", "id", id, mHelper->getSurfaceString(*setSfState),
                         VT_SRV_LOG_I);
        ...
        loop_counter++;
             ...
             (new_mode & (*setSfState)) != new_mode);  // all surface need in this mode is set

    if (VT_SRV_CALL_4G == mode) {
        // We need to make sure that
        // pause/resume operation cannot overlapping with operation (ex: setCamera) between
        // different MAs or the camera status inside MA may get into a mess
        ...
        bool oldTxIsOn = ((curr_state == VT_SRV_MA_STATE_START_UD) ||
                          (curr_state == VT_SRV_MA_STATE_START_UI) ||
                          (curr_state == VT_SRV_MA_STATE_START_UN));
        bool newTxIsOn = ((new_mode == VT_SRV_DATA_PATH_SOURCE) ||
                          (new_mode == VT_SRV_DATA_PATH_SOURCE_SINK));
        bool oldRxIsOn = ((curr_state == VT_SRV_MA_STATE_START_UD) ||
                          (curr_state == VT_SRV_MA_STATE_START_ID) ||
                          (curr_state == VT_SRV_MA_STATE_START_ND));
        bool newRxIsOn =
                ((new_mode == VT_SRV_DATA_PATH_SINK) || (new_mode == VT_SRV_DATA_PATH_SOURCE_SINK));

        mHelper->logFlow("updateCallMode", "oldTxIsOn", id, mHelper->getOnOffString(oldTxIsOn),
                         VT_SRV_LOG_I);
        mHelper->logFlow("updateCallMode", "newTxIsOn", id, mHelper->getOnOffString(newTxIsOn),
                         VT_SRV_LOG_I);
        mHelper->logFlow("updateCallMode", "oldRxIsOn", id, mHelper->getOnOffString(oldRxIsOn),
                         VT_SRV_LOG_I);
        mHelper->logFlow("updateCallMode", "newRxIsOn", id, mHelper->getOnOffString(newRxIsOn),
                         VT_SRV_LOG_I);
...
status_t VTCore::deinit(const int id) {
    ...
    if (mLastRet == VT_SRV_RET_OK) {
        ...
        notifyCallback(id, VT_SRV_NOTIFY_UPLINK_STATE_CHANGE,
                       VT_SRV_VTCAM_STATE_STOP_REC_AND_PREVIEW, 0, 0, String8(""), String8(""),
                       NULL);
...
status_t VTCore::close_internal(const int id, int close_mode) {
    ...
    mHelper->logFlow("close internal", "id", id, mHelper->getVideoStateString(close_mode),
                     VT_SRV_LOG_I);
    ...
    if (mLastRet) {
        ...
        if (VT_SRV_RET_OK == mHelper->check(id) && VT_SRV_MA_STATE_STOPED == state) {
            ...
            mLastRet = mHelper->getParam(id, VT_SRV_PARAM_UPDATE_INFO,
            ...
            mHelper->logFlow("close internal", "current call count", mHelper->getUsedNr(), "",
                             VT_SRV_LOG_I);

            notifyCallback(id, VT_SRV_NOTIFY_CALL_END);
        } else {
            mHelper->logFlow("close internal", "id", id,
                             "has close or deleted, do nothing just skip", VT_SRV_LOG_E);
    ...
    if (VT_SRV_CALL_4G == mode) {
        ...
        if (close_mode == MA_SOURCE) {
            ...
            mHelper->getParam(id, VT_SRV_PARAM_IS_RECV_UPDATE,
                              reinterpret_cast<void**>(&isRecvUpdate));
...
status_t VTCore::setHandoverStateBySimId(int sim_id, bool state) {
    ...
    int idx = 0;
    mHelper->logAction("setHandoverStateBySimId", "id", sim_id, mHelper->getHOString(state),
                       VT_SRV_LOG_I);
...
status_t VTCore::setHandoverStateByNetworkId(int network_id, bool state) {
    ...
    int idx = 0;
    mHelper->logAction("setHandoverStateByNetworkId", "id", network_id, mHelper->getHOString(state),
                       VT_SRV_LOG_I);
...
status_t VTCore::setPreviewSurface(int id, const sp<VTSurface>& surface) {
    ...
    mHelper->logAction("setPreviewSurface", "state (before)", id,
...
status_t VTCore::setDisplaySurface(int id, const sp<VTSurface>& surface) {
    ...
    mHelper->logAction("setDisplaySurface", "state (before)", id,
                       mHelper->getSurfaceString((*state)), VT_SRV_LOG_I);
...
status_t VTCore::setCameraParameters(int sim_id, int sensorCnt, sensor_info_vilte_t* sensor) {
    ...
    for (int i = 0; i < sensorCnt; i++) {
        VT_LOGI("sensor[%d]: id=%d, width=%d, height=%d, degree=%d, facing=%d, hal=%d", i,
                sensor[i].index, sensor[i].max_width, sensor[i].max_height, sensor[i].degree,
                sensor[i].facing, sensor[i].hal);
    }
...
status_t VTCore::setCameraParametersWithSim(int sim_id, int major_sim_id, int sensorCnt,
...
status_t VTCore::setUIMode(int id, VT_SRV_UI_MODE mode) {
    ...
    if (mode == VT_SRV_UI_MODE_FG) {
        ...
        mLastRet = mHelper->getParam(id, VT_SRV_PARAM_PEER_UI_CONFIG,
        ...
        if (config->mWidth == 0 || config->mHeight == 0) {
            ...
            notifyCallback(id, VT_SRV_NOTIFY_PEER_SIZE_CHANGED, config->mWidth, config->mHeight,
                           config->mRotation, String8(""), String8(""), NULL);
        }
...
status_t VTCore::requestCancelSessionModify(int id, sp<VideoProfile> Vp) {
    ...
    mLastRet = mHelper->setParam(id, VT_SRV_PARAM_CANCEL_SESSION_REQ,
                                 reinterpret_cast<void*>(&request));
    if (mLastRet) goto error;

    mLastRet = VT_Send(VT_SRV_CALL_4G, MSG_ID_WRAP_IMSVT_IMCB_MODIFY_SESSION_CANCEL_REQ,
                       reinterpret_cast<void*>(&request), sizeof(VT_IMCB_CANCEL_REQ));
...
status_t VTCore::requestSessionModify(int id, const String8& config) {
    ...
    if ((VT_SRV_RET_OK == mLastRet) && (*isForceStop) == VT_TRUE) {
        mHelper->logAction("requestSessionModify", "id", id, "isForceStopd, ignore session modify",
                           VT_SRV_LOG_W);
        return VT_SRV_RET_OK;
    }
    ...
    mHelper->logAction("requestSessionModify", "id", id,
                       mHelper->getVideoStateString(Vp->getState()), VT_SRV_LOG_I);
    ...
    mLastRet = VT_Send(VT_SRV_CALL_4G, MSG_ID_WRAP_IMSVT_IMCB_MODIFY_SESSION_REQ,
                       reinterpret_cast<void*>(&request), sizeof(VT_IMCB_REQ));
    if (mLastRet) goto error;
...
status_t VTCore::responseSessionModify(int id, const String8& config) {
    ...
    mHelper->logAction("responseSessionModify", "id", id,
                       mHelper->getVideoStateString(Vp->getState()), VT_SRV_LOG_I);
    ...
    mLastRet = VT_Send(VT_SRV_CALL_4G, MSG_ID_WRAP_IMSVT_IMCB_MODIFY_SESSION_RSP,
                       reinterpret_cast<void*>(&response), sizeof(VT_IMCB_RSP));
    if (mLastRet) goto error;
...
status_t VTCore::stopRecording(int id) { return VT_SRV_RET_OK; }
...
status_t VTCore::retriggerUpdate(int id) {
    ...
    mHelper->logFlow("retriggerUpdate", "ua->setting.mode", id,
                     mHelper->getCallModeString(ua->setting.mode), VT_SRV_LOG_I);
    mHelper->logFlow("retriggerUpdate", "info->mIsHold", id, mHelper->getOnOffString(info->mIsHold),
                     VT_SRV_LOG_I);
    mHelper->logFlow("retriggerUpdate", "info->mIsHeld", id, mHelper->getOnOffString(info->mIsHeld),
                     VT_SRV_LOG_I);
    mHelper->logFlow("retriggerUpdate", "info->mIsResume", id,
                     mHelper->getOnOffString(info->mIsResume), VT_SRV_LOG_I);
    mHelper->logFlow("retriggerUpdate", "info->mIsResumed", id,
                     mHelper->getOnOffString(info->mIsResumed), VT_SRV_LOG_I);
    mHelper->logFlow("retriggerUpdate", "info->mIsTxInPause", id,
                     mHelper->getOnOffString(info->mIsTxInPause), VT_SRV_LOG_I);
    mHelper->logFlow("retriggerUpdate", "info->mIsRxInPause", id,
...
status_t VTCore::updateNetworkTable(bool is_add, int network_id, String8 if_name) {
    if (is_add) {
        VT_LOGI("[SRV] [OPERATION][NETWORK_ID=%d] Add if_name:%s", network_id,
                (char*)if_name.string());
...
void VTCore::notifyCallback(int32_t id, int32_t msgType) {
    notifyCallback(id, msgType, 0, 0, 0, String8(""), String8(""), NULL);
}

void VTCore::notifyCallback(int32_t id, int32_t msgType, int32_t arg1, int32_t arg2, int32_t arg3,
    ...
    mHelper->logMsg(msgType, "", id, arg1, arg2, arg3, obj1.string(), obj2.string(), VT_SRV_LOG_I);
    ...
    if (msgType == VT_SRV_NOTIFY_AVPF_TMMBR_MBR_DL) {
        ...
        VT_Send(VT_SRV_CALL_4G, MSG_ID_WRAP_IMSVT_IMCB_MOD_BW_REQ,
                reinterpret_cast<void*>(&g_vt.ma_bw), sizeof(VT_IMCB_BW));
        return;
        ...
        VT_Send(VT_SRV_CALL_4G, MSG_ID_MD_ANBR_REPORT_REQ, reinterpret_cast<void*>(&ANBR_REQ),
                sizeof(VT_ANBR_REQ));
        return;
    }
...
void VTCore::notifyCallback(int id, int msg, int arg1, int arg2) {
    notifyCallback(id, msg, arg1, arg2, 0, String8(""), String8(""), NULL);
}

void VTCore::resetUpdateInfo(int id) { return; }
...
void VTCore::getUpdateInfo(int id, VT_IMCB_UPD* pre_config, VT_IMCB_UPD* new_config) {
    ...
    mHelper->logAction("getUpdateInfo", "old bHold", id,
                       mHelper->getOnOffString(pre_config->config.bHold), VT_SRV_LOG_I);
    mHelper->logAction("getUpdateInfo", "new bHold", id,
                       mHelper->getOnOffString(new_config->config.bHold), VT_SRV_LOG_I);
    mHelper->logAction("getUpdateInfo", "old bHeld", id,
                       mHelper->getOnOffString(pre_config->config.bHeld), VT_SRV_LOG_I);
    mHelper->logAction("getUpdateInfo", "new bHeld", id,
                       mHelper->getOnOffString(new_config->config.bHeld), VT_SRV_LOG_I);
...
void vt_callback(int type, void* data, int len) {
    ...
    if (type == MSG_ID_WRAP_IMSVT_IMCB_GET_CAP_IND) {
        ...
        helper->logAction("vt_callback", "id", id, "MSG_ID_WRAP_IMSVT_IMCB_GET_CAP_IND",
                          VT_SRV_LOG_W);
        VT_LOGW("[SRV] [OPERATION][ID=%d] vt_callback (MSG_ID_WRAP_IMSVT_IMCB_GET_CAP_IND) opid = "
                "%d",
                id, opid);
        ...
        if (opid > 0) {
            ...
            core->notifyCallback(id, VT_SRV_NOTIFY_GET_CAP_WITH_SIM,
        ...
        while (count < 100 && helper->check(id) != VT_SRV_RET_OK) {
            helper->logAction("vt_callback", "MSG_ID_WRAP_IMSVT_IMCB_CONFIG_INIT_IND", id,
                              "this id is not init yet, wait for CC FWK", VT_SRV_LOG_E);
            usleep(200 * 1000);
            count++;
        }
        ...
        helper->logAction("vt_callback", "MSG_ID_WRAP_IMSVT_IMCB_CONFIG_INIT_IND, mode = ", id,
                          helper->getCallModeString(ua.setting.mode), VT_SRV_LOG_W);
        ...
        bool* isForceStop;
        ret = helper->getParam(id, VT_SRV_PARAM_IS_FORCE_STOP,
                               reinterpret_cast<void**>(&isForceStop));

        if (ret) goto error;

        if ((*isForceStop) == VT_TRUE) {
            helper->logAction("vt_callback", "id", id,
                              "(MSG_ID_WRAP_IMSVT_IMCB_CONFIG_UPDATE_IND : isForceStopd !)",
                              VT_SRV_LOG_W);
            return;
        }
        ...
        if ((*isMaCrash) == VT_TRUE) {
            helper->logAction("vt_callback", "id", id,
                              "(MSG_ID_WRAP_IMSVT_IMCB_CONFIG_UPDATE_IND : isMaCrash !)",
                              VT_SRV_LOG_W);
            return;
        }
        ...
        helper->logAction("vt_callback", "rtp_direction (before)", id,
                          helper->getVideoStateString(ua.config.rtp_direction), VT_SRV_LOG_W);
        helper->logAction("vt_callback", "early_media_direction", id,
                          helper->getVideoStateString(ua.setting.early_media_direction),
                          VT_SRV_LOG_W);

        VT_BOOL* isDuringEarlyMedia;
        helper->getParam(id, VT_SRV_PARAM_IS_DURING_EARLY_MEDIA,
                         reinterpret_cast<void**>(&isDuringEarlyMedia));

        if (!helper->isCallConnected(id) && ua.setting.early_media_direction != 0 &&
            ua.config.rtp_direction != 0) {
            helper->logAction("vt_callback", "id", id, "notify virtual early media enter",
                              VT_SRV_LOG_W);
            ...
            if ((*isDuringEarlyMedia) == VT_FALSE) {
                ...
                sp<VideoProfile> Vp =
                        helper->geteEarlyMediaProfileFromState(ua.config.rtp_direction);
                core->notifyCallback(id, VT_SRV_NOTIFY_RECV_ENHANCE_SESSION_IND,
                                     VT_SRV_SESSION_IND_EARLY_MEDIA,
                                     1,  // early media start
                                     0, helper->packFromVdoProfile(Vp), String8(""), NULL);
            }
            ...
            helper->logAction("vt_callback", "id", id, "notify virtual early media leave",
                              VT_SRV_LOG_W);
        ...
        ret = helper->setParam(id, VT_SRV_PARAM_IS_RECV_UPDATE,
                               reinterpret_cast<void*>(&isRecvUpdate));
        if (ret) goto error;
        ...
        id = CONSTRUCT_SIM_CALL_ID(ua.setting.sim_slot_id, ua.config.call_id);
        helper->logAction("vt_callback", "id", id, "MSG_ID_WRAP_IMSVT_IMCB_CONFIG_DEINIT_IND",
                          VT_SRV_LOG_W);
        ...
        helper->logAction("vt_callback", "id", id, "MSG_ID_WRAP_IMSVT_IMCB_MODIFY_SESSION_IND",
                          VT_SRV_LOG_W);

        VT_BOOL* isForceCancel;
        int ret = helper->getParam(id, VT_SRV_PARAM_IS_FORCE_CANCEL,
                                   reinterpret_cast<void**>(&isForceCancel));
        if ((VT_SRV_RET_OK == ret) && (*isForceCancel) == VT_TRUE) {
            helper->logFlow("MSG_ID_WRAP_IMSVT_IMCB_MODIFY_SESSION_IND", "clear force cancel!",
                            VT_IVD, "", VT_SRV_LOG_I);
            *isForceCancel = VT_FALSE;
        }
        ...
        helper->logAction("vt_callback", "Vp->getState", id,
                          helper->getVideoStateString(Vp->getState()), VT_SRV_LOG_W);

        ret = helper->setParam(id, VT_SRV_PARAM_REMOTE_SESSION_REQ,
                               reinterpret_cast<void*>(&Indication));
        if (ret) goto error;

        core->notifyCallback(id, VT_SRV_NOTIFY_RECV_SESSION_CONFIG_REQ, 0, 0, 0,
                             helper->packFromVdoProfile(Vp), String8(""), NULL);
        ...
        helper->logAction("vt_callback", "id", id, "MSG_ID_WRAP_IMSVT_IMCB_MODIFY_SESSION_CNF",
                          VT_SRV_LOG_W);

        sp<VideoProfile> spVp_confirm = helper->getProfileFromCnf(&confrim);

        helper->logAction("vt_callback", "spVp_confirm->getState()", id,
                          helper->getVideoStateString(spVp_confirm->getState()), VT_SRV_LOG_W);
        helper->logAction("vt_callback", "spVp_confirm->getResult()", id,
                          helper->getSessionModifyResultString(spVp_confirm->getResult()),
                          VT_SRV_LOG_W);

        sp<VideoProfile> spVp_request = helper->getRequestVdoProfile(id);

        helper->logAction("vt_callback", "spVp_request->getState", id,
                          helper->getVideoStateString(spVp_request->getState()), VT_SRV_LOG_W);
        ...
        core->notifyCallback(id, VT_SRV_NOTIFY_RECV_SESSION_CONFIG_RSP, spVp_confirm->getResult(),
                             0, 0, helper->packFromVdoProfile(spVp_request),
                             helper->packFromVdoProfile(spVp_confirm), NULL);

        spVp_request = NULL;
        spVp_confirm = NULL;

    } else if (type == MSG_ID_WRAP_IMSVT_IMCB_HANDOVER_START_IND) {
        helper->logAction("vt_callback", "MSG_ID_WRAP_IMSVT_IMCB_HANDOVER_START_IND", VT_IVD, "",
                          VT_SRV_LOG_W);
        ...
        helper->logAction("vt_callback", "MSG_ID_WRAP_IMSVT_IMCB_HANDOVER_STOP_IND", VT_IVD, "",
                          VT_SRV_LOG_W);
        ...
        helper->logAction("vt_callback", "MSG_ID_WRAP_IMSVT_IMCB_PDN_HANDOVER_BEGIN_IND", VT_IVD,
                          "", VT_SRV_LOG_W);
        ...
        helper->logAction("vt_callback", "MSG_ID_WRAP_IMSVT_IMCB_PDN_HANDOVER_END_IND", VT_IVD, "",
                          VT_SRV_LOG_W);
        ...
        helper->logAction("vt_callback", "id", id, "MSG_ID_WRAP_IMSVT_IMCB_MODIFY_CANCEL_IND",
                          VT_SRV_LOG_W);
        ...
        ret = helper->getParam(id, VT_SRV_PARAM_IS_FORCE_CANCEL,
                               reinterpret_cast<void**>(&mIsForceCancel));
        if (ret) {
        ...
        core->notifyCallback(id, VT_SRV_NOTIFY_RECV_ENHANCE_SESSION_IND, VT_SRV_SESSION_IND_CANCEL,
                             0, 0, helper->packFromVdoProfile(Vp), String8(""), NULL);

    } else if (type == MSG_ID_WRAP_IMSVT_IMCB_MODIFY_SESSION_CANCEL_CNF) {
        helper->logAction("vt_callback", "MSG_ID_WRAP_IMSVT_IMCB_MODIFY_CANCEL_CNF", VT_IVD, "",
                          VT_SRV_LOG_W);
        ...
        VT_IMCB_CANCEL_REQ* request;
        ret = helper->getParam(id, VT_SRV_PARAM_CANCEL_SESSION_REQ,
                               reinterpret_cast<void**>(&request));
        if (ret) goto error;

        core->notifyCallback(id, VT_SRV_NOTIFY_RECV_SESSION_CONFIG_RSP, Vp->getResult(), 0, 0,
                             helper->packFromVdoProfile(Vp), helper->packFromVdoProfile(Vp), NULL);

    } else if (type == MSG_ID_WRAP_IMSVT_IMCB_EVENT_LOCAL_BW_READY_IND) {
        helper->logAction("vt_callback", "MSG_ID_WRAP_IMSVT_IMCB_EVENT_LOCAL_BW_READY_IND", VT_IVD,
                          "", VT_SRV_LOG_W);
        ...
        core->notifyCallback(id, VT_SRV_NOTIFY_BW_READY_IND);
        ...
        helper->logAction("vt_callback", "id", id, "MSG_ID_WRAP_IMSVT_IMCB_CALL_PROGRESS_IND",
                          VT_SRV_LOG_W);
        ...
        (*callState) = ind.ecpi;
        helper->logAction("vt_callback", "callState", id, helper->getCallStateString(*callState),
                          VT_SRV_LOG_W);

    } else if (type == MSG_ID_MD_ANBR_CONFIG_UPDATE_IND) {
        helper->logAction("vt_callback", "MSG_ID_MD_ANBR_CONFIG_UPDATE_IND", VT_IVD, "",
                          VT_SRV_LOG_W);
        VT_ANBR_CONFIG anbr;
        ...
        VT_LOGI("[SRV] [VT_ANBR_CONFIG] config=%d, ebi=%d, is_ul=%d, bitrate=%d, bearer_id=%d, "
                "pdu_session_id=%d, ext_param=%d",
                anbr.config, anbr.ebi, anbr.is_ul, anbr.bitrate, anbr.bearer_id,
                anbr.pdu_session_id, anbr.ext_param);
        ...
        helper->logAction("vt_callback", "MSG_ID_MD_INTER_RAT_STATUS_IND", VT_IVD, "",
                          VT_SRV_LOG_W);
        VT_IRAT_STRUCT irat;
        ...
        VT_LOGI("[SRV] [MSG_ID_MD_INTER_RAT_STATUS_IND] sim_slot_id=%d, irat_status=%d, "
                "is_successful=%d",
         ...
         *     EIRAT: 0, 0/1 -> stop
         */
        if (irat.irat_status == 0 && (irat.is_successful == 0 || irat.is_successful == 1) &&
...
void vt_rtp_codec_2_ua(int mode, VT_IMCB_CAP* ua, int ssid, int opid) {
    if (mode == VT_SRV_CALL_4G) {
        ...
        ImsMa::getCodecParameterSets(VIDEO_H264, &levelCapNumbers, &codeclevelcap, opid,
                                     QUALITY_FINE);

        VT_LOGI("[SRV] [vt_rtp_codec_2_ua] (H264) levelCapNumbers = %d (need <= %d)",
                levelCapNumbers, VT_MAX_PS_NUM);

        for (int i = 0; i < levelCapNumbers && i < VT_MAX_PS_NUM; i++) {
            ua->h264_ps[i].profile_level_id =
                    codeclevelcap[i].codec_level_fmtp.h264_codec_level_fmtp.profile_level_id;
            memcpy(ua->h264_ps[i].sprop_parameter_sets,
                   codeclevelcap[i].codec_level_fmtp.h264_codec_level_fmtp.sprop_parameter_sets,
                   VT_MAX_SDP_PARAMETER_SET_LENGTH);
        }

        ImsMa::getCodecParameterSets(VIDEO_HEVC, &levelCapNumbers, &codeclevelcap, opid,
                                     QUALITY_FINE);

        VT_LOGI("[SRV] [vt_rtp_codec_2_ua] (H265) levelCapNumbers = %d (need <= %d)",
                levelCapNumbers, VT_MAX_PS_NUM);

        for (int i = 0; i < levelCapNumbers && i < VT_MAX_PS_NUM; i++) {
            ua->hevc_ps[i].profile_id =
                    codeclevelcap[i].codec_level_fmtp.hevc_codec_level_fmtp.profile_id;
            ua->hevc_ps[i].level_id =
                    codeclevelcap[i].codec_level_fmtp.hevc_codec_level_fmtp.level_id;
            memcpy(ua->hevc_ps[i].sprop_vps,
                   codeclevelcap[i].codec_level_fmtp.hevc_codec_level_fmtp.sprop_vps,
                   VT_MAX_SDP_PARAMETER_SET_LENGTH);
            memcpy(ua->hevc_ps[i].sprop_sps,
                   codeclevelcap[i].codec_level_fmtp.hevc_codec_level_fmtp.sprop_sps,
                   VT_MAX_SDP_PARAMETER_SET_LENGTH);
            memcpy(ua->hevc_ps[i].sprop_pps,
                   codeclevelcap[i].codec_level_fmtp.hevc_codec_level_fmtp.sprop_pps,
                   VT_MAX_SDP_PARAMETER_SET_LENGTH);
            memcpy(ua->hevc_ps[i].sprop_sei,
                   codeclevelcap[i].codec_level_fmtp.hevc_codec_level_fmtp.sprop_sei,
                   VT_MAX_SDP_PARAMETER_SET_LENGTH);
        }
        ...
        VT_LOGI("[SRV] [vt_rtp_codec_2_ua] bit rate table Cnt     = %d (need <= %d)", tableCnt,
                VT_MAX_LEVEL_SIZE);
        ...
        for (int i = 0; i < codec_num; i++) {
            for (int j = 0; j < rtp_num; j++) {
                if (codecCap[i].format == rtpCap[j].mime_Type) {
                    VT_LOGI("[SRV] [vt_rtp_codec_2_ua] find match type = %d, count = %d",
                            rtpCap[j].mime_Type, final_num);

                    vt_srv_video_capability_t* cap = &(ua->video_cap[final_num]);

                    cap->mime_Type = codecCap[i].format;
                    if (cap->mime_Type == VIDEO_H264) {
                        cap->codec_cap.h264_codec.profile_level_id =
                                codecCap[i].codec_fmtp.h264_codec_fmtp.profile_level_id;
                        cap->codec_cap.h264_codec.max_recv_level =
                                codecCap[i].codec_fmtp.h264_codec_fmtp.max_recv_level;
                        cap->codec_cap.h264_codec.redundant_pic_cap =
                                codecCap[i].codec_fmtp.h264_codec_fmtp.redundant_pic_cap;
                        memcpy(cap->codec_cap.h264_codec.sprop_parameter_sets,
                               codecCap[i].codec_fmtp.h264_codec_fmtp.sprop_parameter_sets,
                               VOLTE_MAX_SDP_PARAMETER_SET_LENGTH);
                        memcpy(cap->codec_cap.h264_codec.sprop_level_parameter_sets,
                               codecCap[i].codec_fmtp.h264_codec_fmtp.sprop_level_parameter_sets,
                               VOLTE_MAX_SDP_PARAMETER_SET_LENGTH);
                        cap->codec_cap.h264_codec.max_mbps =
                                codecCap[i].codec_fmtp.h264_codec_fmtp.max_mbps;
                        cap->codec_cap.h264_codec.max_smbps =
                                codecCap[i].codec_fmtp.h264_codec_fmtp.max_smbps;
                        cap->codec_cap.h264_codec.max_fs =
                                codecCap[i].codec_fmtp.h264_codec_fmtp.max_fs;
                        cap->codec_cap.h264_codec.max_cpb =
                                codecCap[i].codec_fmtp.h264_codec_fmtp.max_cpb;
                        cap->codec_cap.h264_codec.max_dpb =
                                codecCap[i].codec_fmtp.h264_codec_fmtp.max_dpb;
                        cap->codec_cap.h264_codec.max_br =
                                codecCap[i].codec_fmtp.h264_codec_fmtp.max_br;
                        cap->codec_cap.h264_codec.max_rcmd_nalu_size =
                                codecCap[i].codec_fmtp.h264_codec_fmtp.max_rcmd_nalu_size;
                        cap->codec_cap.h264_codec.sar_understood =
                                codecCap[i].codec_fmtp.h264_codec_fmtp.sar_understood;
                        cap->codec_cap.h264_codec.sar_supported =
                                codecCap[i].codec_fmtp.h264_codec_fmtp.sar_supported;
                        ...
                        for (int k = 0; k < VT_MAX_IMAGE_NUM; k++) {
                            ...
                            memcpy(cap->codec_cap.h264_codec.image_send[k].sar,
                                   codecCap[i].image_send[k].sar, VT_MAX_SDP_EGBYTE_LENGTH);
                            memcpy(cap->codec_cap.h264_codec.image_send[k].par,
                                   codecCap[i].image_send[k].par, VT_MAX_SDP_EGBYTE_LENGTH);
                            memcpy(cap->codec_cap.h264_codec.image_send[k].q,
                                   codecCap[i].image_send[k].q, VT_MAX_SDP_EGBYTE_LENGTH);
                        }
                        for (int k = 0; k < VT_MAX_IMAGE_NUM; k++) {
                            ...
                            memcpy(cap->codec_cap.h264_codec.image_recv[k].sar,
                                   codecCap[i].image_recv[k].sar, VT_MAX_SDP_EGBYTE_LENGTH);
                            memcpy(cap->codec_cap.h264_codec.image_recv[k].par,
                                   codecCap[i].image_recv[k].par, VT_MAX_SDP_EGBYTE_LENGTH);
                            memcpy(cap->codec_cap.h264_codec.image_recv[k].q,
                                   codecCap[i].image_recv[k].q, VT_MAX_SDP_EGBYTE_LENGTH);
                        }

                    } else if (cap->mime_Type == VIDEO_HEVC) {
                        cap->codec_cap.hevc_codec.profile_space =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.profile_space;
                        cap->codec_cap.hevc_codec.profile_id =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.profile_id;
                        cap->codec_cap.hevc_codec.tier_flag =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.tier_flag;
                        cap->codec_cap.hevc_codec.level_id =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.level_id;

                        for (int k = 0; k < 6; k++) {
                            cap->codec_cap.hevc_codec.interop_constraints[k] =
                                    codecCap[i].codec_fmtp.hevc_codec_fmtp.interop_constraints[k];
                        }
                        for (int k = 0; k < 4; k++) {
                            cap->codec_cap.hevc_codec.profile_comp_ind[k] =
                                    codecCap[i].codec_fmtp.hevc_codec_fmtp.profile_comp_ind[k];
                        }

                        cap->codec_cap.hevc_codec.sprop_sub_layer_id =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.sprop_sub_layer_id;
                        cap->codec_cap.hevc_codec.recv_sub_layer_id =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.recv_sub_layer_id;
                        cap->codec_cap.hevc_codec.max_recv_level_id =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.max_recv_level_id;
                        cap->codec_cap.hevc_codec.tx_mode =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.tx_mode;

                        memcpy(cap->codec_cap.hevc_codec.sprop_vps,
                               codecCap[i].codec_fmtp.hevc_codec_fmtp.sprop_vps,
                               VOLTE_MAX_SDP_PARAMETER_SET_LENGTH);
                        memcpy(cap->codec_cap.hevc_codec.sprop_sps,
                               codecCap[i].codec_fmtp.hevc_codec_fmtp.sprop_sps,
                               VOLTE_MAX_SDP_PARAMETER_SET_LENGTH);
                        memcpy(cap->codec_cap.hevc_codec.sprop_pps,
                               codecCap[i].codec_fmtp.hevc_codec_fmtp.sprop_pps,
                               VOLTE_MAX_SDP_PARAMETER_SET_LENGTH);
                        memcpy(cap->codec_cap.hevc_codec.sprop_sei,
                               codecCap[i].codec_fmtp.hevc_codec_fmtp.sprop_sei,
                               VOLTE_MAX_SDP_PARAMETER_SET_LENGTH);

                        cap->codec_cap.hevc_codec.max_lsr =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.max_lsr;
                        cap->codec_cap.hevc_codec.max_lps =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.max_lps;
                        cap->codec_cap.hevc_codec.max_cpb =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.max_cpb;
                        cap->codec_cap.hevc_codec.max_dpb =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.max_dpb;
                        cap->codec_cap.hevc_codec.max_br =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.max_br;
                        cap->codec_cap.hevc_codec.max_tr =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.max_tr;
                        cap->codec_cap.hevc_codec.max_tc =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.max_tc;
                        cap->codec_cap.hevc_codec.max_fps =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.max_fps;
                        cap->codec_cap.hevc_codec.sprop_max_don_diff =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.sprop_max_don_diff;
                        cap->codec_cap.hevc_codec.sprop_depack_buf_nalus =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.sprop_depack_buf_nalus;
                        cap->codec_cap.hevc_codec.sprop_depack_buf_bytes =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.sprop_depack_buf_bytes;
                        cap->codec_cap.hevc_codec.depack_buf_cap =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.depack_buf_cap;
                        cap->codec_cap.hevc_codec.sprop_seg_id =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.sprop_seg_id;
                        cap->codec_cap.hevc_codec.sprop_spatial_seg_idc =
                                codecCap[i].codec_fmtp.hevc_codec_fmtp.sprop_spatial_seg_idc;

                        memcpy(cap->codec_cap.hevc_codec.dec_parallel_cap,
                               codecCap[i].codec_fmtp.hevc_codec_fmtp.dec_parallel_cap,
                               VOLTE_MAX_SDP_PARAMETER_SET_LENGTH);
                        memcpy(cap->codec_cap.hevc_codec.include_dph,
                               codecCap[i].codec_fmtp.hevc_codec_fmtp.include_dph,
                               VOLTE_MAX_SDP_PARAMETER_SET_LENGTH);
                        ...
                        for (int k = 0; k < VT_MAX_IMAGE_NUM; k++) {
                            ...
                            memcpy(cap->codec_cap.hevc_codec.image_send[k].sar,
                                   codecCap[i].image_send[k].sar, VT_MAX_SDP_EGBYTE_LENGTH);
                            memcpy(cap->codec_cap.hevc_codec.image_send[k].par,
                                   codecCap[i].image_send[k].par, VT_MAX_SDP_EGBYTE_LENGTH);
                            memcpy(cap->codec_cap.hevc_codec.image_send[k].q,
                                   codecCap[i].image_send[k].q, VT_MAX_SDP_EGBYTE_LENGTH);
                        }
                        for (int k = 0; k < VT_MAX_IMAGE_NUM; k++) {
                            ...
                            memcpy(cap->codec_cap.hevc_codec.image_recv[k].sar,
                                   codecCap[i].image_recv[k].sar, VT_MAX_SDP_EGBYTE_LENGTH);
                            memcpy(cap->codec_cap.hevc_codec.image_recv[k].par,
                                   codecCap[i].image_recv[k].par, VT_MAX_SDP_EGBYTE_LENGTH);
                            memcpy(cap->codec_cap.hevc_codec.image_recv[k].q,
                                   codecCap[i].image_recv[k].q, VT_MAX_SDP_EGBYTE_LENGTH);
                        }
                    }
                    ...
                    memcpy(cap->rtcp_fb_type, rtpCap[j].rtcp_fb_type,
                           sizeof(rtcp_fb_param_type_t) * VT_MAX_RTCP_FB_SIZE);

                    cap->extmap_num = rtpCap[j].rtp_header_extension_num;
                    VT_LOGI("[SRV] [vt_rtp_codec_2_ua] extmap_num             = %d",
                            cap->extmap_num);
                    for (int k = 0; k < cap->extmap_num; k++) {
                        ...
                        memcpy(cap->extmap[k].ext_uri, rtpCap[j].rtp_ext_map[k].extension_uri,
                               VT_MAX_EXTMAP_URI_SIZE);
                        VT_LOGI("[SRV] [vt_rtp_codec_2_ua] ext_id                 = %d",
                                cap->extmap[k].ext_id);
                        VT_LOGI("[SRV] [vt_rtp_codec_2_ua] direction              = %d",
                                cap->extmap[k].direction);
                        VT_LOGI("[SRV] [vt_rtp_codec_2_ua] ext_uri                = %s",
                                cap->extmap[k].ext_uri);
                    }
                    VT_LOGI("[SRV] [vt_rtp_codec_2_ua] cap.media_type         = %d",
                            cap->media_type);
                    VT_LOGI("[SRV] [vt_rtp_codec_2_ua] cap.mime_Type          = %d",
                            cap->mime_Type);
...
void vt_ua_2_rtp(int mode, VT_IMCB_CONFIG* ua, rtp_rtcp_config_t* rtp) {
    if (mode == VT_SRV_CALL_4G) {
        ...
        memcpy(rtp->rtcp_fb_type, cap->rtcp_fb_type,
               sizeof(vt_srv_rtcp_fb_param_type_t) * VT_MAX_RTCP_FB_SIZE);
...
void vt_ua_2_codec(int mode, VT_IMCB_CONFIG* ua, video_codec_fmtp_t* codec) {
    if (mode == VT_SRV_CALL_4G) {
        ...
        VT_LOGI("[SRV] [vt_ua_2_codec] profile_level_id      = %d",
                codec_cfg->h264_codec.profile_level_id);
        ...
        if (VIDEO_H264 == codec->format) {
            ...
            codec->codec_fmtp.h264_codec_fmtp.profile_level_id =
                    codec_cfg->h264_codec.profile_level_id;
            codec->codec_fmtp.h264_codec_fmtp.max_recv_level = codec_cfg->h264_codec.max_recv_level;
            codec->codec_fmtp.h264_codec_fmtp.redundant_pic_cap =
                    codec_cfg->h264_codec.redundant_pic_cap;

            memcpy(codec->codec_fmtp.h264_codec_fmtp.sprop_parameter_sets,
                   codec_cfg->h264_codec.sprop_parameter_sets, VT_MAX_SDP_PARAMETER_SET_LENGTH);
            memcpy(codec->codec_fmtp.h264_codec_fmtp.sprop_level_parameter_sets,
                   codec_cfg->h264_codec.sprop_level_parameter_sets,
                   VT_MAX_SDP_PARAMETER_SET_LENGTH);
            ...
            codec->codec_fmtp.h264_codec_fmtp.max_rcmd_nalu_size =
                    codec_cfg->h264_codec.max_rcmd_nalu_size;
            ...
            VT_LOGI("[SRV] [vt_ua_2_codec] profile_id            = %d",
                    codec_cfg->hevc_codec.profile_id);
            ...
            for (int i = 0; i < 6; i++) {
                codec->codec_fmtp.hevc_codec_fmtp.interop_constraints[i] =
                        codec_cfg->hevc_codec.interop_constraints[i];
            }

            for (int i = 0; i < 4; i++) {
                codec->codec_fmtp.hevc_codec_fmtp.profile_comp_ind[i] =
                        codec_cfg->hevc_codec.profile_comp_ind[i];
            }

            codec->codec_fmtp.hevc_codec_fmtp.sprop_sub_layer_id =
                    codec_cfg->hevc_codec.sprop_sub_layer_id;
            codec->codec_fmtp.hevc_codec_fmtp.recv_sub_layer_id =
                    codec_cfg->hevc_codec.recv_sub_layer_id;
            codec->codec_fmtp.hevc_codec_fmtp.max_recv_level_id =
                    codec_cfg->hevc_codec.max_recv_level_id;
            codec->codec_fmtp.hevc_codec_fmtp.tx_mode = codec_cfg->hevc_codec.tx_mode;

            memcpy(codec->codec_fmtp.hevc_codec_fmtp.sprop_vps, codec_cfg->hevc_codec.sprop_vps,
                   VOLTE_MAX_SDP_PARAMETER_SET_LENGTH);
            memcpy(codec->codec_fmtp.hevc_codec_fmtp.sprop_sps, codec_cfg->hevc_codec.sprop_sps,
                   VOLTE_MAX_SDP_PARAMETER_SET_LENGTH);
            memcpy(codec->codec_fmtp.hevc_codec_fmtp.sprop_pps, codec_cfg->hevc_codec.sprop_pps,
                   VOLTE_MAX_SDP_PARAMETER_SET_LENGTH);
            memcpy(codec->codec_fmtp.hevc_codec_fmtp.sprop_sei, codec_cfg->hevc_codec.sprop_sei,
                   VOLTE_MAX_SDP_PARAMETER_SET_LENGTH);
            ...
            codec->codec_fmtp.hevc_codec_fmtp.sprop_max_don_diff =
                    codec_cfg->hevc_codec.sprop_max_don_diff;
            codec->codec_fmtp.hevc_codec_fmtp.sprop_depack_buf_nalus =
                    codec_cfg->hevc_codec.sprop_depack_buf_nalus;
            codec->codec_fmtp.hevc_codec_fmtp.sprop_depack_buf_bytes =
                    codec_cfg->hevc_codec.sprop_depack_buf_bytes;
            ...
            codec->codec_fmtp.hevc_codec_fmtp.sprop_spatial_seg_idc =
                    codec_cfg->hevc_codec.sprop_spatial_seg_idc;

            memcpy(codec->codec_fmtp.hevc_codec_fmtp.dec_parallel_cap,
                   codec_cfg->hevc_codec.dec_parallel_cap, VOLTE_MAX_SDP_PARAMETER_SET_LENGTH);
            memcpy(codec->codec_fmtp.hevc_codec_fmtp.include_dph, codec_cfg->hevc_codec.include_dph,
                   VOLTE_MAX_SDP_PARAMETER_SET_LENGTH);
...
void vt_ut_ua_2_rtp(int mode, VT_IMCB_CAP* ua, rtp_rtcp_config_t* rtp) {
    if (mode == VT_SRV_CALL_4G) {
        ...
        rtp->rtcp_fb_param_num = cap->rtcp_fb_param_num;
        memcpy(rtp->rtcp_fb_type, cap->rtcp_fb_type,
               sizeof(VT_MAX_RTCP_FB_SIZE) * sizeof(vt_srv_rtcp_fb_param_type_t));
        ...
        for (int i = 0; i < rtp->rtp_header_extension_num; i++) {
            ...
            memcpy(rtp->rtp_ext_map[i].extension_uri, cap->extmap[i].ext_uri,
                   VT_MAX_EXTMAP_URI_SIZE);
        }
