...
status_t VTMALStub::Init(ma_datapath_t type) {
    ...
    if (MA_INVALID_MODE == mMode) {
        getLogger()->logAction("VTMALStub", "id", mId, "Init (mode = invalid) (Finish)",
                               VT_SRV_LOG_W);
...
status_t VTMALStub::Start(ma_datapath_t type) {
    ...
    if (MA_INVALID_MODE == mMode) {
        getLogger()->logAction("VTMALStub", "id", mId, "Start (mode = invalid) (Finish)",
                               VT_SRV_LOG_W);
        return VT_SRV_RET_ERR_MA;
        ...
        if (type & MA_SOURCE) {
            mVTCore->notifyCallback(getId(), VT_SRV_NOTIFY_UPLINK_STATE_CHANGE,
                                    VT_SRV_VTCAM_STATE_START_REC, 0, 0, String8(""), String8(""),
                                    NULL);
            ...
            mVTCore->notifyCallback(getId(), VT_SRV_NOTIFY_LOCAL_BUFFER, 0, 0, 0, String8(""),
...
status_t VTMALStub::Pause(ma_datapath_t type, imsma_pause_resume_params_t* params) {
    ...
    if (MA_INVALID_MODE == mMode) {
        getLogger()->logAction("VTMALStub", "id", mId, "Pause (mode = invalid) (Finish)",
                               VT_SRV_LOG_W);
        return VT_SRV_RET_ERR_MA;
...
status_t VTMALStub::Resume(ma_datapath_t type, imsma_pause_resume_params_t* params) {
    ...
    if (MA_INVALID_MODE == mMode) {
        getLogger()->logAction("VTMALStub", "id", mId, "Resume (mode = invalid) (Finish)",
                               VT_SRV_LOG_W);
        return VT_SRV_RET_ERR_MA;
...
status_t VTMALStub::Reset(ma_datapath_t type) {
    ...
    if (MA_INVALID_MODE == mMode) {
        getLogger()->logAction("VTMALStub", "id", mId, "Reset (mode = invalid) (Finish)",
                               VT_SRV_LOG_W);
        return VT_SRV_RET_ERR_MA;
...
status_t VTMALStub::UpdateMediaConfig(media_config_t* config) {
    if (MA_INVALID_MODE == mMode) {
        ...
        if (mImsMa->UpdateMediaConfig(config)) {
            getLogger()->logAction("VTMALStub", "id", mId, "UpdateMediaConfig, error",
                                   VT_SRV_LOG_E);
            return VT_SRV_RET_ERR_MA;
        } else {
            return VT_SRV_RET_OK;
...
status_t VTMALStub::getBufferQueueProducer(sp<IGraphicBufferProducer>* outBufferProducer) {
    if (MA_INVALID_MODE == mMode) {
        ...
        if (mImsMa->getBufferQueueProducer(outBufferProducer)) {
            getLogger()->logAction("VTMALStub", "id", mId, "getBufferQueueProducer, error",
                                   VT_SRV_LOG_E);
            return VT_SRV_RET_ERR_MA;
        } else {
            return VT_SRV_RET_OK;
...
status_t VTMALStub::SetRecordParameters(record_quality_t quality, char* file_name) {
    if (MA_INVALID_MODE == mMode) {
        ...
        if (mImsMa->SetRecordParameters(quality, file_name)) {
            getLogger()->logAction("VTMALStub", "id", mId, "SetRecordParameters, error",
                                   VT_SRV_LOG_E);
            return VT_SRV_RET_ERR_MA;
        } else {
            return VT_SRV_RET_OK;
...
status_t VTMALStub::setDeviceRotationDegree(int32_t degree) {
    if (MA_INVALID_MODE == mMode) {
        ...
        if (mImsMa->setDeviceRotationDegree(degree)) {
            getLogger()->logAction("VTMALStub", "id", mId, "setDeviceRotationDegree, error",
                                   VT_SRV_LOG_E);
            return VT_SRV_RET_ERR_MA;
        } else {
            return VT_SRV_RET_OK;
...
void ImsMaHandler::finalization(void) { mUser.clear(); }
...
void ImsMaHandler::onMessageReceived(const sp<AMessage>& msg) {
    ...
    switch (reason) {
        ...
        case kWhatPeerResolutionDegree: {
            ...
            VTCore::mHelper->setParam(mId, VT_SRV_PARAM_PEER_UI_CONFIG,
                                      reinterpret_cast<void*>(&config));

            mUser->notifyCallback(mId, VT_SRV_NOTIFY_PEER_SIZE_CHANGED, w, h, r, String8(""),
                                  String8(""), NULL);
        } break;
        case kWhatLocalResolutionDegree: {
            ...
            VTCore::mHelper->setParam(mId, VT_SRV_PARAM_LOCAL_UI_CONFIG,
                                      reinterpret_cast<void*>(&config));

            mUser->notifyCallback(mId, VT_SRV_NOTIFY_LOCAL_SIZE_CHANGED, w, h, r, String8(""),
                                  String8(""), NULL);
        } break;
