...
void VTCoreHelper::logAction(const char* action, const char* des, int data, const char* data2,
                             VT_SRV_LOG_LV level) {
    char msg[VT_SRV_STR_LEN];
    ...
    if (!strcmp(des, "id")) {
        ...
        if (data == VT_IVD) {
            ...
            snprintf(msg, VT_SRV_STR_LEN, "%s[ID=%d] %s (%s : %s)", prefix, data, action, des,
                     data2);
        }
    }
...
void VTCoreHelper::logFlow(const char* flow, const char* des, int data, const char* data2,
                           VT_SRV_LOG_LV level) {
    char msg[VT_SRV_STR_LEN];
...
void VTCoreHelper::logMsg(int notify, const char* des, int id, int data1, int data2, int data3,
                          const char* obj1, const char* obj2, VT_SRV_LOG_LV level) {
    char msg[VT_SRV_STR_LEN];
    ...
    if (!strcmp(des, "")) {
        snprintf(msg, VT_SRV_STR_LEN, "%s %s (id = %d) (arg1 = %d, arg2 = %d, arg3 = %d)", prefix,
                 type, id, data1, data2, data3);
        snprintf(msg2, VT_SRV_STR_LEN, "%s                   (obj1 = %s, obj2 = %s)", prefix, obj1,
                 obj2);
    } else {
...
int VTCoreHelper::getId(int idx) { return mMap.mTable[idx].mId; }
...
int VTCoreHelper::isUsed(int idx) { return mMap.mUsed[idx]; }
...
status_t VTCoreHelper::setHandoverStateBySim(int simId, int state) {
    if (0 <= simId && simId < VT_SRV_SIM_NR) {
        VT_LOGD("[SRV]   [MAP OPERATION] [SIM ID=%d] setHandOverStateBySim (set state:%d)", simId,
                state);
    ...
    for (int i = 0; i < VT_SRV_MA_NR; i++) {
        if (VT_SRV_RET_OK == isMatch(i, id)) {
            if (type == VT_SRV_PARAM_UA_CONFIG) {
                ...
                mMap.mTable[i].mSetSurfaceState =
                        (*(reinterpret_cast<VT_SRV_SURFACE_STATE*>(param)));
...
bool VTCoreHelper::isCallConnected(int id) {
    ...
    if (*callState == VT_CALL_STATE_CONNECTED || *callState == VT_CALL_STATE_ACTIVE ||
        *callState == VT_CALL_STATE_HELD || *callState == VT_CALL_STATE_REMOTE_HOLD ||
    ...
    if (isDebug()) {
        for (int i = 0; i < VT_SRV_MA_NR; i++) {
            if (-1 != mMap.mTable[i].mId) {
                ...
                VT_LOGW("[SRV]   [MAP DUMP DATA]     (SIM id            %d )",
                        mMap.mTable[i].mSimId);
                VT_LOGW("[SRV]   [MAP DUMP DATA]     (ptr               %d )",
                        mMap.mTable[i].mPtr.get());
                VT_LOGW("[SRV]   [MAP DUMP DATA]     (state             %s )",
                        getStateString(mMap.mTable[i].mState));
                VT_LOGW("[SRV]   [MAP DUMP DATA]     (surface state     %s )",
                        getSurfaceString(mMap.mTable[i].mSetSurfaceState));
                VT_LOGW("[SRV]   [MAP DUMP DATA]     (IsForceStop       %d )",
                        mMap.mTable[i].mIsForceStop);
                VT_LOGW("[SRV]   [MAP DUMP DATA]     (has receive init  %d )",
                        mMap.mTable[i].mHasReceiveInit);
...
String8 VTCoreHelper::packFromVdoProfile(sp<VideoProfile> videoProfile) {
    ...
    snprintf(config, VT_SRV_STR_LEN, "mVideoState=%d;mQuality=%d", videoProfile->getState(),
             videoProfile->getQty());
...
sp<VideoProfile> VTCoreHelper::getProfileFromCnf(VT_IMCB_CNF* confirm) {
    ...
    VT_LOGW("[SRV] [getProfileFromCnf]     (mode = %d, cam = %d)", confirm->mode,
            confirm->camera_direction);
...
sp<VideoProfile> VTCoreHelper::getProfileFromInd(VT_IMCB_IND* indication) {
    ...
    VT_LOGW("[SRV] [getProfileFromInd]     (mode = %d, cam = %d)", indication->mode,
            indication->camera_direction);
...
int VTCoreHelper::getLabOperator() { return getIntProperty("persist.vendor.vt.lab_op_code"); }
...
int VTCoreHelper::getCurrentOperator(int simId) {
    ...
    if (VT_SIM_ID_ABSENT == simId) {
        VT_LOGI("[SRV] getCurrentOperator finish : SIM absent, return default op code = %d",
                VT_DEFAULT_OP_ID);
        return VT_DEFAULT_OP_ID;
    }
    ...
    do {
        ...
        if (op == VT_NO_MATCHED_OP_ID) {
            VT_LOGI("[SRV] getCurrentOperator cannot match opid:%d, return default op code = %d",
                    op, VT_DEFAULT_OP_ID);
            op = VT_DEFAULT_OP_ID;
            break;
        }
    ...
    if (op == 0) {
        VT_LOGI("[SRV] getCurrentOperator cannot match opid:%d, return default op code = %d", op,
                VT_DEFAULT_OP_ID);
        op = VT_DEFAULT_OP_ID;
    }
...
status_t VTCoreHelper::setRequestVdoProfile(int id, sp<VideoProfile> vdo_profile) {
    ...
    for (int i = 0; i < VT_SRV_MA_NR; i++) {
        if (VT_SRV_RET_OK == isMatch(i, id)) {
            ...
            logMap("setRequestVdoProfile", getVideoStateString(vdo_profile->getState()), id,
                   VT_SRV_LOG_I);
            return VT_SRV_RET_OK;
        }
    }
    ...
    for (int i = 0; i < VT_SRV_MA_NR; i++) {
        if (VT_SRV_RET_OK == isMatch(i, id)) {
            logMap("getRequestVdoProfile",
                   getVideoStateString(mMap.mTable[i].mSessionReqProfile->getState()), id,
                   VT_SRV_LOG_I);
...
void VTCoreHelper::addNetworkInfoRecord(int network_id, String8 if_name) {
    ...
    if (idx == VT_INVALID_INDEX) {
        VT_LOGI("[SRV]   [MAP OPERATION]  [ID=%d] AddNetworkInfoRecord ifname=%s", network_id,
                (char*)if_name.string());

        mMap.mNetworkInfoTable.push_back(net_info);

    } else {
        VT_LOGI("[SRV]   [MAP OPERATION] [ID=%d] UpdateNetworkInfoRecord ifname=%s", network_id,
                (char*)if_name.string());
...
int VTCoreHelper::getNetworkIdByIfName(String8 if_name) {
    ...
    for (int i = 0; i < mMap.mNetworkInfoTable.size(); i++) {
        if (!strcmp(mMap.mNetworkInfoTable[i].mIfName, if_name.string())) {
            logMap("checkNetworkInfRecordExist", "entry exist",
                   mMap.mNetworkInfoTable[i].mNetworkId, VT_SRV_LOG_I);
...
int VT_Send(int target, int type, void* data, int len) {
    ...
    g_vt_hidl_write_fmq
            ->writeBlocking((uint8_t*)data, len,
                            static_cast<uint32_t>(IVideoTelephony::EventFlagBits::FMQ_NOT_FULL),
                            static_cast<uint32_t>(IVideoTelephony::EventFlagBits::FMQ_NOT_EMPTY),
                            VT_HIDL_READ_WRITE_WAITTING_TIME /* timeOutNanos */
                                                             /*,
...
static void* VT_HIDL_Thread(void* arg) {
    ...
    while (1) {
        ...
        ret =
                g_vt_hidl_read_fmq->readBlocking(
                        (uint8_t*)(&msg_type), 4,
                        static_cast<uint32_t>(IVideoTelephony::EventFlagBits::FMQ_NOT_FULL),
                        static_cast<uint32_t>(IVideoTelephony::EventFlagBits::FMQ_NOT_EMPTY),
                        VT_HIDL_READ_WRITE_WAITTING_TIME /* timeOutNanos */
