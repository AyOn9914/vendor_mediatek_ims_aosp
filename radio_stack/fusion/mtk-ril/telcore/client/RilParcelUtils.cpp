...
/** Callee expects NULL */
void
dispatchVoid(RIL_RequestFunc onRequest, Parcel& p __unused, RfxRequestInfo *pRI) {
...
void
invalidCommandBlock(RfxRequestInfo *pRI) {
    RFX_LOG_E(LOG_TAG, "invalid command block for token %d request %s",
        pRI->token, mtkRequestToString(pRI->pCI->requestNumber));
}

/** Callee expects const char * */
void
dispatchString(RIL_RequestFunc onRequest, Parcel& p, RfxRequestInfo *pRI) {
    status_t status;
    size_t datalen;
    size_t stringlen;
    char *string8 = NULL;
    ...
    onRequest(pRI->pCI->requestNumber, (void *)string8,
...
return;
 ...
 /** Callee expects const char ** */
void
dispatchStrings(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
...
void
dispatchInts(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    int32_t count = 0;
    ...
    if (pInts == NULL) {
        RFX_LOG_E(LOG_TAG, "Memory allocation failed for request %s", mtkRequestToString(pRI->pCI->requestNumber));
        return;
    }
    ...
    onRequest(pRI->pCI->requestNumber, const_cast<int *>(pInts),
 ...
 */
void
dispatchSmsWrite (RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    ...
    startRequest;
    appendPrintBuf("%s%d,%s,smsc=%s", tempPrintBuf, args.status,
 ...
 */
void
dispatchDial (RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    RIL_Dial dial;
    ...
    if (uusPresent) {
        appendPrintBuf("%s,uusType=%d,uusDcs=%d,uusLen=%d", tempPrintBuf,
 ...
 */
void
dispatchEmergencyDial (RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
 ...
 */
void
dispatchSIM_IO (RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    union RIL_SIM_IO {
        RIL_SIM_IO_v6 v6;
        RIL_SIM_IO_v5 v5;
    } simIO;
 ...
 */
void
dispatchSIM_APDU (RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    int32_t t = 0;
    status_t status;
    RIL_SIM_APDU apdu;
    ...
    appendPrintBuf("%ssessionid=%d,cla=%d,ins=%d,p1=%d,p2=%d,p3=%d,data=%s",
        tempPrintBuf, apdu.sessionid, apdu.cla, apdu.instruction, apdu.p1, apdu.p2,
 ...
 */
void
dispatchCallForward(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    ...
    startRequest;
    appendPrintBuf("%sstat=%d,reason=%d,serv=%d,toa=%d,%s,tout=%d", tempPrintBuf,
 ...
 */
void
dispatchCallForwardEx(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
...
void
dispatchRaw(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    int32_t len;
    status_t status;
    const void *data;
...
status_t
constructCdmaSms(Parcel &p, RfxRequestInfo *pRI __unused, RIL_CDMA_SMS_Message& rcsm) {
    int32_t  t = 0;
    uint8_t ut = 0;
    status_t status;
    int32_t digitCount;
...
void
dispatchCdmaSms(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
...
void
dispatchImsCdmaSms(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI, uint8_t retry, int32_t messageRef) {
...
void
dispatchImsGsmSms(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI, uint8_t retry, int32_t messageRef) {
    RIL_IMS_SMS_Message rism;
    int32_t countStrings;
    status_t status;
    size_t datalen;
    ...
    startRequest;
    appendPrintBuf("%stech=%d, retry=%d, messageRef=%d, ", tempPrintBuf,
    ...
    if (countStrings == 0) {
        ...
        RFX_LOG_E(LOG_TAG, "ImsGsmSms smsc %s, pdu %s", ((pStrings[0] != NULL)? pStrings[0] : "null"),
...
void
dispatchImsSms(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    int32_t  t;
...
void
dispatchCdmaSmsAck(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    ...
    appendPrintBuf("%suErrorClass=%d, uTLStatus=%d, ",
            tempPrintBuf, rcsa.uErrorClass, rcsa.uSMSCauseCode);
    closeRequest;
...
void
dispatchGsmBrSmsCnf(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    int32_t t = 0;
    status_t status;
    int32_t num;
...

    {
        ...
        for (int i = 0 ; i < num ; i++ ) {
            ...
            appendPrintBuf("%s [%d: fromServiceId=%d, toServiceId =%d, \
                  fromCodeScheme=%d, toCodeScheme=%d, selected =%d]", tempPrintBuf, i,
...
void
dispatchCdmaBrSmsCnf(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    int32_t t = 0;
    status_t status = NO_ERROR;
    int32_t num = 0;
...

    {
        ...
        for (int i = 0 ; i < num ; i++ ) {
            ...
            appendPrintBuf("%s [%d: service_category=%d, language =%d, \
                  entries.bSelected =%d]", tempPrintBuf, i, cdmaBci[i].service_category,
...
void dispatchVoiceRadioTech(RIL_RequestFunc onRequest __unused, Parcel& p __unused, RfxRequestInfo *pRI __unused) {
#if 0
...
// When all RILs handle this request, this function can be removed and
// the request can be sent directly to the RIL using dispatchVoid.
void dispatchCdmaSubscriptionSource(RIL_RequestFunc onRequest __unused, Parcel& p __unused, RfxRequestInfo *pRI __unused) {
#if 0
    RIL_RadioState state = CALL_ONSTATEREQUEST((RIL_SOCKET_ID)pRI->socket_id);
...
void dispatchSetInitialAttachApn(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI)
{
    ...
    appendPrintBuf("%sapn=%s, protocol=%s, authtype=%d, username=%s, password=%s",
            tempPrintBuf, pf.apn, pf.protocol, pf.authtype, pf.username, pf.password);
    closeRequest;
    printRequest(pRI->token, pRI->pCI->requestNumber);
...
void dispatchNVWriteItem(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    ...
    startRequest;
    appendPrintBuf("%snvwi.itemID=%d, value=%s, ", tempPrintBuf, nvwi.itemID,
            nvwi.value);
    closeRequest;
...
void dispatchUiccSubscripton(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    ...
    appendPrintBuf("slot=%d, app_index=%d, act_status = %d", uicc_sub.slot, uicc_sub.app_index,
            uicc_sub.act_status);
    RFX_LOG_D(LOG_TAG, "dispatchUiccSubscription, slot=%d, app_index=%d, act_status = %d", uicc_sub.slot,
...

    {
        ...
        for (int i = 0 ; i < num ; i++ ) {
            ...
            appendPrintBuf("%s [%d: profileId=%d, apn =%s, protocol =%s, authType =%d, \
                  ...
                  waitTime =%d, enabled =%d]", tempPrintBuf, i, dataProfiles[i].profileId,
...
void dispatchRadioCapability(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI){
    ...
    appendPrintBuf("%s [version:%d, session:%d, phase:%d, rat:%d, \
            logicalModemUuid:%s, status:%d", tempPrintBuf, rc.version, rc.session,
...
void dispatchCarrierRestrictions(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
    ...
    appendPrintBuf("%s len_allowed_carriers:%d, len_excluded_carriers:%d,",
                   tempPrintBuf, cr.len_allowed_carriers, cr.len_excluded_carriers);
    ...
    for (int32_t i = 0; i < cr.len_allowed_carriers; i++) {
        ...
        appendPrintBuf("%s [%d mcc:%s, mnc:%s, match_type:%d, match_data:%s],",
                       tempPrintBuf, i, p_cr->mcc, p_cr->mnc, p_cr->match_type, p_cr->match_data);
    }
...
void dispatchSetSignalStrengthReportingCriteria(RIL_RequestFunc onRequest, Parcel &p, RfxRequestInfo *pRI) {
...
int responseFailCause(Parcel &p, void *response, size_t responselen) {
    ...
    if (responselen == sizeof(int)) {
      ...
      appendPrintBuf("%s[cause_code=%d,vendor_cause=%s]", tempPrintBuf, p_fail_cause_info->cause_code,
      ...
      RFX_LOG_E(LOG_TAG, "responseFailCause: invalid response length %d expected an int or "
...
int responseSMS(Parcel &p, void *response, size_t responselen) {
    ...
    startResponse;
    appendPrintBuf("%s%d,%s,%d", tempPrintBuf, p_cur->messageRef,
...
int responseDataCallListV4(Parcel &p, void *response, size_t responselen)
{
    ...
    if (responselen % sizeof(RIL_Data_Call_Response_v4) != 0) {
        RFX_LOG_E(LOG_TAG, "responseDataCallListV4: invalid response length %d expected multiple of %d",
    ...
    for (i = 0; i < num; i++) {
        ...
        appendPrintBuf("%s[cid=%d,%s,%s,%s],", tempPrintBuf,
            p_cur[i].cid,
...
int responseDataCallListV6(Parcel &p, void *response, size_t responselen)
{
    ...
    if (responselen % sizeof(RIL_Data_Call_Response_v6) != 0) {
        RFX_LOG_E(LOG_TAG, "responseDataCallListV6: invalid response length %d expected multiple of %d",
...
int responseDataCallListV9(Parcel &p, void *response, size_t responselen)
{
    ...
    if (responselen % sizeof(RIL_Data_Call_Response_v9) != 0) {
        RFX_LOG_E(LOG_TAG, "responseDataCallListV9: invalid response length %d expected multiple of %d",
...
int responseDataCallListV11(Parcel &p, void *response, size_t responselen) {
    ...
    if (responselen % sizeof(MTK_RIL_Data_Call_Response_v11) != 0) {
        RFX_LOG_E(LOG_TAG, "invalid response length %d expected multiple of %d",
    ...
    for (i = 0; i < num; i++) {
        ...
        appendPrintBuf("%s[status=%d,retry=%d,cid=%d,%s,%s,%s,%s,%s,%s,%s,%d,rat=%d],", tempPrintBuf,
        p_cur[i].status,
        p_cur[i].suggestedRetryTime,
        p_cur[i].cid,
        (p_cur[i].active==0)?"down":"up",
...
int responseCallForwards(Parcel &p, void *response, size_t responselen) {
    ...
    if (responselen % sizeof(RIL_CallForwardInfo *) != 0) {
        RFX_LOG_E(LOG_TAG, "responseCallForwards: invalid response length %d expected multiple of %d",
...
int responseCallForwardsEx(Parcel &p __unused, void *response __unused, size_t responselen __unused) {
#if 0
    int num;
...
int responseCellList(Parcel &p, void *response, size_t responselen) {
    ...
    for (int i = 0 ; i < num ; i++) {
        ...
        appendPrintBuf("%s[cid=%s,rssi=%d],", tempPrintBuf,
...
int responseCdmaInformationRecords(Parcel &p,
    ...
    if (responselen != sizeof (RIL_CDMA_InformationRecords)) {
        RFX_LOG_E(LOG_TAG, "responseCdmaInformationRecords: invalid response length %d expected multiple of %d\n",
        ...
        switch (infoRec->name) {
            ...
            case RIL_CDMA_EXTENDED_DISPLAY_INFO_REC:
                if (infoRec->rec.display.alpha_len >
                    ...
                    RFX_LOG_E(LOG_TAG, "invalid display info response length %d \
                ...
                if (string8 == NULL) {
                    RFX_LOG_E(LOG_TAG, "Memory allocation failed for responseCdmaInformationRecords");
                    closeRequest;
            ...
            case RIL_CDMA_LINE_CONTROL_INFO_REC:
                ...
                appendPrintBuf("%slineCtrlPolarityIncluded=%d, \
                                ...
                                lineCtrlPowerDenial=%d, ", tempPrintBuf,
...
int responseCdmaSignalInfoRecord(Parcel &p, void *response, size_t responselen) {
    ...
    if (responselen != sizeof (RIL_CDMA_SignalInfoRecord)) {
        RFX_LOG_E(LOG_TAG, "invalid response length %d expected sizeof (RIL_CDMA_SignalInfoRecord) of %d\n",
...
int responseCellInfoListV6(Parcel &p, void *response, size_t responselen) {
    ...
    if (responselen % sizeof(RIL_CellInfo) != 0) {
        RFX_LOG_E(LOG_TAG, "responseCellInfoList: invalid response length %d expected multiple of %d",
...
int responseHardwareConfig(Parcel &p, void *response, size_t responselen)
{
   ...
   if (responselen % sizeof(RIL_HardwareConfig) != 0) {
       RFX_LOG_E(LOG_TAG, "responseHardwareConfig: invalid response length %d expected multiple of %d",
   ...
   for (i = 0; i < num; i++) {
      switch (p_cur[i].type) {
         case RIL_HARDWARE_CONFIG_MODEM: {
            ...
            appendPrintBuf("%s modem: uuid=%s,state=%d,rat=%08x,maxV=%d,maxD=%d,maxS=%d", tempPrintBuf,
...
int responseGetSmsSimMemStatusCnf(Parcel &p,void *response, size_t responselen)
{
    ...
    if (responselen != sizeof (RIL_SMS_Memory_Status)) {
        RFX_LOG_E(LOG_TAG, "invalid response length %d expected sizeof (RIL_SMS_Memory_Status) of %d\n",
...
int responseEtwsNotification(Parcel &p __unused, void *response __unused, size_t responselen __unused) {
#if 0
    if(NULL == response) {
        RFX_LOG_E(LOG_TAG, "invalid response: NULL");
        return RIL_ERRNO_INVALID_RESPONSE;
...
int responseCarrierRestrictions(Parcel &p, void *response, size_t responselen) {
  ...
  if (responselen != sizeof(RIL_CarrierRestrictionsWithPriority)) {
    RFX_LOG_E(LOG_TAG, "responseCarrierRestrictions: invalid response length %u expecting len: %u",
...
int responseVoiceRegistrationState(Parcel &p, void *response, size_t responselen) {
    ...
    if (responselen != sizeof(RIL_VoiceRegistrationStateResponse)) {
        RFX_LOG_E(LOG_TAG, "invalid response length was %d expected %d",
             (int)responselen, (int)sizeof (RIL_VoiceRegistrationStateResponse));
        return RIL_ERRNO_INVALID_RESPONSE;
    }
...
int responseDataRegistrationState(Parcel &p, void *response, size_t responselen) {
    ...
    if (responselen != sizeof(RIL_DataRegistrationStateResponse)) {
        RFX_LOG_E(LOG_TAG, "invalid response length was %d expected %d",
             (int)responselen, (int)sizeof (RIL_DataRegistrationStateResponse));
        return RIL_ERRNO_INVALID_RESPONSE;
    }
...
int responseEmergencyList(Parcel &p, void *response, size_t responselen) {
    ...
    for (i = 0; i < num; i++) {
        ...
        appendPrintBuf("%s[%s,%s,%s,%d,%d],", printBuf,
            p_cur[i].number, p_cur[i].mcc, p_cur[i].mnc, p_cur[i].categories, p_cur[i].sources);
    }
    removeLastChar;
    closeResponse;
    printResponse;
