...
size_t BlobCache::getFlattenedSize() const {
    size_t size = align4(sizeof(Header) + MTK_PROPERTY_VALUE_MAX);
...
status_t BlobCache::flatten(void* buffer, size_t size) const {
    ...
    char buildId[MTK_PROPERTY_VALUE_MAX];
    mtk_property_get("ro.build.id", buildId, "");
    header->mBuildIdLength = strlen(buildId);
...
status_t BlobCache::unflatten(void const* buffer, size_t size) {
    ...
    char buildId[MTK_PROPERTY_VALUE_MAX];
    int len = mtk_property_get("ro.build.id", buildId, "");
