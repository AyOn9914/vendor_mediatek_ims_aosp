...
/// M: [2635 telematics] internal add
const char ipv6_proc_path[] = "/proc/sys/net/ipv6/conf";

/* MTK start*/
#include <stdlib.h>
const char TC_PATH[] = "tc";
int rx = -1;
int tx = -1;
/* MTK end */
...
int ifc_init(void)
{
    ...
    if (DBG) ALOGE("ifc_init_returning %d", ret);
    return ret;
}
...
int ifc_set_addr(const char *name, in_addr_t addr)
{
    ...
    if (DBG) ALOGE("ifc_set_addr(%s, xx) = %d", name, ret);
    return ret;
}
...
int ifc_set_mask(const char *name, in_addr_t mask)
{
    ...
    if (DBG) ALOGE("ifc_set_mask(%s, xx) = %d", name, ret);
    return ret;
}
...
int ifc_create_default_route(const char *name, in_addr_t gw)
{
    ...
    if (DBG) ALOGE("ifc_create_default_route(%s, %d) = %d", name, gw, ret);
    return ret;
}
...
ifc_configure(const char *ifname,
    ...
    mtk_property_set(dns_prop_name, dns1 ? ipaddr_to_string(dns1) : "");
    snprintf(dns_prop_name, sizeof(dns_prop_name), "net.%s.dns2", ifname);
    mtk_property_set(dns_prop_name, dns2 ? ipaddr_to_string(dns2) : "");
...
static int setEnableIPv6(char* ifname, int on) {
    ...
    if (ret <= 0 || ((unsigned int)ret != size)) {
        ALOGE("Failed to write %s: ret=%d, errno=%d:%s", path, ret, errno, strerror(errno));
        close(fd);
        return -1;
    }
    close(fd);
...
int ifc_setInterfaceThrottle(const char *iface, int rxKbps, int txKbps) {
    ...
    /*
     * by mtk80842, ifc_reset configuration before setting
     */
    ifc_reset(ifn);
    ...
    ALOGE("ifc_setInterfaceThrottle success, ifn = %s\n", ifn);
