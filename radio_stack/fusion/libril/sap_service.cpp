...
struct SapImpl : android::hardware::radio::V1_2::ISap {
    int32_t slotId;
...
void SapImpl::checkReturnStatus(Return<void>& ret) {
    if (ret.isOk() == false) {
        mtkLogE(LOG_TAG, "checkReturnStatus: unable to call response/indication callback: %s",
...
Return<void> SapImpl::setCallback(const ::android::sp<ISapCallback>& sapCallbackParam) {
    mtkLogD(LOG_TAG, "SapImpl::setCallback for slotId %d", slotId);
...
Return<void> SapImpl::connectReq(int32_t token, int32_t maxMsgSize) {
    ...
    mtkLogD(LOG_TAG, "SapImpl::connectReq calling pb_encode");
    if (!pb_encode(&stream, RIL_SIM_SAP_CONNECT_REQ_fields, &req)) {
        mtkLogE(LOG_TAG, "SapImpl::connectReq: Error encoding RIL_SIM_SAP_CONNECT_REQ");
...
Return<void> SapImpl::disconnectReq(int32_t token) {
    ...
    mtkLogD(LOG_TAG, "SapImpl::disconnectReq calling pb_encode");
    if (!pb_encode(&stream, RIL_SIM_SAP_DISCONNECT_REQ_fields, &req)) {
        mtkLogE(LOG_TAG, "SapImpl::disconnectReq: Error encoding RIL_SIM_SAP_DISCONNECT_REQ");
...
Return<void> SapImpl::apduReq(int32_t token, SapApduType type, const hidl_vec<uint8_t>& command) {
    ...
    if (!pb_get_encoded_size(&encodedSize, RIL_SIM_SAP_APDU_REQ_fields, &req)) {
        mtkLogE(LOG_TAG, "SapImpl::apduReq: Error getting encoded size for RIL_SIM_SAP_APDU_REQ");
    ...
    mtkLogD(LOG_TAG, "SapImpl::apduReq calling pb_encode");
    if (!pb_encode(&stream, RIL_SIM_SAP_APDU_REQ_fields, &req)) {
        mtkLogE(LOG_TAG, "SapImpl::apduReq: Error encoding RIL_SIM_SAP_APDU_REQ");
...
Return<void> SapImpl::transferAtrReq(int32_t token) {
    ...
    mtkLogD(LOG_TAG, "SapImpl::transferAtrReq calling pb_encode");
    if (!pb_encode(&stream, RIL_SIM_SAP_TRANSFER_ATR_REQ_fields, &req)) {
        mtkLogE(LOG_TAG, "SapImpl::transferAtrReq: Error encoding RIL_SIM_SAP_TRANSFER_ATR_REQ");
...
Return<void> SapImpl::powerReq(int32_t token, bool state) {
    ...
    if (!pb_get_encoded_size(&encodedSize, RIL_SIM_SAP_POWER_REQ_fields, &req)) {
        mtkLogE(LOG_TAG, "SapImpl::powerReq: Error getting encoded size for RIL_SIM_SAP_POWER_REQ");
    ...
    mtkLogD(LOG_TAG, "SapImpl::powerReq calling pb_encode");
    if (!pb_encode(&stream, RIL_SIM_SAP_POWER_REQ_fields, &req)) {
        mtkLogE(LOG_TAG, "SapImpl::powerReq: Error encoding RIL_SIM_SAP_POWER_REQ");
...
Return<void> SapImpl::resetSimReq(int32_t token) {
    ...
    mtkLogD(LOG_TAG, "SapImpl::resetSimReq calling pb_encode");
    if (!pb_encode(&stream, RIL_SIM_SAP_RESET_SIM_REQ_fields, &req)) {
        mtkLogE(LOG_TAG, "SapImpl::resetSimReq: Error encoding RIL_SIM_SAP_RESET_SIM_REQ");
...
void sap::registerService(const RIL_RadioFunctions *callbacks) {
    ...
    for (int i = 0; i < simCount; i++) {
        ...
        mtkLogD(LOG_TAG, "registerService: starting ISap %s for slotId %d", serviceNames[i], i);
        android::status_t status = sapService[i]->registerAsService(serviceNames[i]);
        mtkLogD(LOG_TAG, "registerService: started ISap %s status %d", serviceNames[i], status);
    }
}
