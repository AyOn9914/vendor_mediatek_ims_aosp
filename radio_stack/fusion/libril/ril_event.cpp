...
#if DEBUG
#define dlog(x...) mtkLogD(LOG_TAG, x)
...
void ril_event_loop()
{
    ...
    for (;;) {
        ...
        if (n < 0) {
            ...
            mtkLogE(LOG_TAG, "ril_event: select error (%d)", errno);
            // bail?
            ...
            for (int i=0; (i < MAX_FD_EVENTS); i++) {
                ...
                if(rev != NULL && FD_ISSET(rev->fd, &rfds)) {
                    ...
                    n = select(nfds, &rfds, NULL, NULL, ptv);
                    if(n >=0) {
                        mtkLogE(LOG_TAG, "ril_event: after remove fd: %d, select success", selectFd);
                        break;
                    }
                }
            }
            mtkLogE(LOG_TAG, "ril_event: after remove fd:%d, select success2", selectFd);
                ...
                for(i = 0; i < fd_max; i++) {
                    if (FD_ISSET(i, &readFds)) {
                        ...
                        if(rc < 0) {
                            errcode_test = errno;
                            if(errcode_test == 9) {
                                mtkLogE(LOG_TAG, "ril_event: test error fd= (%d)", i);
                            } else {
                                mtkLogE(LOG_TAG, "ril_event: test error (%d)", errcode_test);
                            }
                        }
                     }
                 }
