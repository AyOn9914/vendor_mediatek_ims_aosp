    ...
    const flat_binder_object& obj, const void* who, size_t* outAshmemSize)
{
    ...
    mtkLogD(LOG_TAG, "Invalid object type 0x%08x", obj.type);
}
    ...
    const flat_binder_object& obj, const void* who, size_t* outAshmemSize)
{
    ...
    mtkLogE(LOG_TAG, "Invalid object type 0x%08x", obj.type);
}
    ...
    const wp<IBinder>& binder, Parcel* out)
{
    ...
    if (binder != NULL) {
        ...
        mtkLogE(LOG_TAG, "Unable to unflatten Binder weak reference!");
        obj.type = BINDER_TYPE_BINDER;
        obj.binder = 0;
        obj.cookie = 0;
...
status_t Parcel::setDataSize(size_t size)
{
    ...
    if (err == NO_ERROR) {
        mDataSize = size;
        mtkLogV(LOG_TAG, "setDataSize Setting data size of %p to %zu", this, mDataSize);
    }
    return err;
}
                              ...
                              IPCThreadState* threadState) const
{
    ...
    if (str == interface) {
        ...
        mtkLogW(LOG_TAG, "**** enforceInterface() expected '%s' but read '%s'",
...
status_t Parcel::finishWrite(size_t len)
{
    ...
    mDataPos += len;
    mtkLogV(LOG_TAG, "finishWrite Setting data pos of %p to %zu", this, mDataPos);
    if (mDataPos > mDataSize) {
        mDataSize = mDataPos;
        mtkLogV(LOG_TAG, "finishWrite Setting data size of %p to %zu", this, mDataSize);
    }
...
status_t Parcel::writeNativeHandle(const native_handle* handle)
{
    ...
    if (err != NO_ERROR) {
        mtkLogD(LOG_TAG, "write native handle, write dup fd failed");
        return err;
    }
...
status_t Parcel::writeBlob(size_t len, bool mutableCopy, WritableBlob* outBlob)
{
    ...
    if (!mAllowFds || len <= BLOB_INPLACE_LIMIT) {
        mtkLogV(LOG_TAG, "writeBlob: write in place");
    ...
    mtkLogV(LOG_TAG, "writeBlob: write to ashmem");
...
{
    mtkAssert((char *)"Parcel::remove() not yet implemented!");
}
...
status_t Parcel::read(void* outData, size_t len) const
{
    ...
    if ((mDataPos+pad_size(len)) >= mDataPos && (mDataPos+pad_size(len)) <= mDataSize
        ...
        mtkLogV(LOG_TAG, "read Setting data pos of %p to %zu", this, mDataPos);
        return NO_ERROR;
    }
...
const void* Parcel::readInplace(size_t len) const
{
    ...
    if ((mDataPos+pad_size(len)) >= mDataPos && (mDataPos+pad_size(len)) <= mDataSize
        ...
        mDataPos += pad_size(len);
        mtkLogV(LOG_TAG, "readInplace Setting data pos of %p to %zu", this, mDataPos);
        return data;
    }
    return NULL;
}
...
const char* Parcel::readCString() const
{
    ...
    if (avail > 0) {
        ...
        if (eos) {
            ...
            mtkLogV(LOG_TAG, "readCString Setting data pos of %p to %zu", this, mDataPos);
            return str;
        }
    }
    return NULL;
...
String16 Parcel::readString16() const
{
    ...
    mtkLogE(LOG_TAG, "Reading a NULL string not supported here.");
    return String16();
}
...
status_t Parcel::readBlob(size_t len, ReadableBlob* outBlob) const
{
    ...
    if (blobType == BLOB_INPLACE) {
        mtkLogV(LOG_TAG, "readBlob: read in place");
    ...
    mtkLogV(LOG_TAG, "readBlob: read from ashmem");
...
status_t Parcel::read(FlattenableHelperInterface& val) const
{
    ...
    for (size_t i=0 ; i<fd_count && err==NO_ERROR ; i++) {
        ...
        if (fds[i] < 0) {
            err = BAD_VALUE;
            mtkLogE(LOG_TAG, "dup() failed in Parcel::read, i is %zu, fds[i] is %d, fd_count is %zu, error: %s",
...
const flat_binder_object* Parcel::readObject(bool nullMetaData) const
{
    ...
    if ((DPOS+sizeof(flat_binder_object)) <= mDataSize) {
        ...
        if (!nullMetaData && (obj->cookie == 0 && obj->binder == 0)) {
            ...
            // reading.
            mtkLogV(LOG_TAG, "readObject Setting data pos of %p to %zu", this, mDataPos);
            return obj;
        }
        ...
        if (N > 0) {
            mtkLogV(LOG_TAG, "Parcel %p looking for obj at %zu, hint=%zu",
                 this, DPOS, opos);
            ...
            if (OBJS[opos] == DPOS) {
                // Found it!
                mtkLogV(LOG_TAG, "Parcel %p found obj %zu at index %zu with forward search",
                     this, DPOS, opos);
            ...
            if (OBJS[opos] == DPOS) {
                // Found it!
                mtkLogV(LOG_TAG, "Parcel %p found obj %zu at index %zu with backward search",
                     this, DPOS, opos);
                mNextObjectHint = opos+1;
                mtkLogV(LOG_TAG, "readObject Setting data pos of %p to %zu", this, mDataPos);
                return obj;
        ...
        mtkLogW(LOG_TAG, "Attempt to read object from Parcel %p at offset %zu that is not in the object list",
             this, DPOS);
    }
    return NULL;
}
    ...
    const binder_size_t* objects, size_t objectsCount, release_func relFunc, void* relCookie)
{
    ...
    mDataPos = 0;
    mtkLogV(LOG_TAG, "setDataReference Setting data pos of %p to %zu", this, mDataPos);
    ...
    for (size_t i = 0; i < mObjectsSize; i++) {
        ...
        if (offset < minOffset) {
            mtkLogE(LOG_TAG, "%s: bad object offset %" PRIu64 " < %" PRIu64 "\n",
...
status_t Parcel::restartWrite(size_t desired)
{
    ...
    mtkLogV(LOG_TAG, "restartWrite Setting data size of %p to %zu", this, mDataSize);
    mtkLogV(LOG_TAG, "restartWrite Setting data pos of %p to %zu", this, mDataPos);
...
status_t Parcel::continueWrite(size_t desired)
{
    ...
    if (mOwner) {
        ...
        mtkLogV(LOG_TAG, "continueWrite Setting data size of %p to %zu", this, mDataSize);
        mDataCapacity = desired;
        ...
        if (desired > mDataCapacity) {
            ...
            if (mDataPos > desired) {
                mDataPos = desired;
                mtkLogV(LOG_TAG, "continueWrite Setting data pos of %p to %zu", this, mDataPos);
            }
        }
        ...
        if(!(mDataCapacity == 0 && mObjects == NULL
             && mObjectsCapacity == 0)) {
            mtkLogE(LOG_TAG, "continueWrite: %zu/%p/%zu/%zu", mDataCapacity, mObjects, mObjectsCapacity, desired);
        }
...
void Parcel::initState()
{
    ...
    mDataCapacity = 0;
    mDataPos = 0;
    mtkLogV(LOG_TAG, "initState Setting data size of %p to %zu", this, mDataSize);
    mtkLogV(LOG_TAG, "initState Setting data pos of %p to %zu", this, mDataPos);
    mObjects = NULL;
    mObjectsSize = 0;
    mObjectsCapacity = 0;
    mNextObjectHint = 0;
